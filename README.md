...
| **Collaborative / Graph** | **Semantic+协同** | **ColaRec** <br>  <br> *使用 T5 编码器 被转化为语义嵌入；使用 LightGCN预训练得到的物品嵌入*  |
...
| | **Multi-identifier item TokenizationE** | **MTGRec** <br>  <br> *将单个RQ-VAE训练过程中相邻epoch的检查点作为多个语义相关的物品标记器。实现了一个物品对应多个语义相关标识符的新范式* |
| **VQ-VAE** | **Multi-Head VQ-VAE** | **LLaDA-Rec** <br>  <br> *提出 Multi-Head VQ-VAE 来生成并行的语义ID。具体做法是：将物品的语义向量分割成M个子向量，每个子向量分别使用一个独立的码本进行量化* |
| | **continuous VQ-VAE** | **ContRec** <br>  <br> *使用σ-VAE Tokenizer生成连续向量作为语义表示的基本单元，并辅助伯努利掩码、K路并行编码器、高斯核约束技术防止坍塌,以“伪令牌”形式直接插入LLM输入序列。包括：词表扩展：在LLM词表中添加两个特殊令牌；嵌入替换：正常文本令牌 → 标准词嵌入查找<z_start>和<z_end> → 学习到的特殊嵌入 中间位置 → 直接使用z_k向量（通过一个投影层适配维度）* |
| | **continuous VQ-VAE** | **Masked Vector-Quantized Tokenizer** <br>  <br> *在传统的VQ-VAE上加入了掩码机制 和 K 路编码器，以提升标记化的泛化能力* |
| | **R-KMeans** | OneRec  |
...
| | **Hierarchical K-Means** | **ColaRec** <br> <br> *使用层次化K-means对CF信号构建对应的GID* |
...
| **Hybrid / Textual** | **BERT Tokenizer** | **GPT4Rec** <br> <br> *把LLM生成的查询序列切分为词元，直接通过BERT转换为ID* |
| **Hybrid / Textual** | **SentencePiece 分词器** | **P5** <br> <br> *将用户ID、物品ID等拆分为多个子词单元（sub-word units），并引入 Whole-Word Embeddings 来标识属于同一原始词的多个子词* |
| **Hybrid / Textual** | **Textual ID** | **GenRec** <br> <br> *使用纯文本ID* |
| **Hybrid / Textual** | **RQ+Clustering-based+Structured Knowledge-based** | **GRACE** <br> <br> *使用 RQ-VAE + K-means 生成层次化语义 token（用于转换隐形特征，如：风格）。使用Chain-of-Thought Tokenization将物品的显式特征转化为离散ID（如：品牌）* |
...
| **Encoder-Decoder (T5/BART)** | **Transformer** | **GRACE** <br>  <br> *在传统的transformer架构的基础上，使用新的注意力机制Journey-Aware Sparse Attention，该机制是一种可学习的稀疏注意力机制，能同时高效捕捉旅程内连续性、旅程间转移性、局部实时性与全局压缩性* |
| **Encoder-Decoder (T5/BART)** | **T5** | **ColaRec** <br>  <br> *把T5用于推荐，并进行微调以适应任务* |
...
| **Decoder-Only (LLM/GPT)** | **LLMs+conditional Denoising Diffusion Probabilistic Model**| **ContRec** <br>  <br> *使用双主干混合架构，扩散过程以LLM的输出为条件，并且使用分散损失* |
...
| **Two-Stage (Quantize $\to$ Train)** | **Curriculum Recommender Pre-training** | **MTGRec** <br> <br> *提出了一种 “Curriculum Recommender Pre-trainin” 范式，通过动态调度来自多个语义相关标记器的扩增数据来训练生成式推荐器。* |
...
| **Constrained Beam Search** | | **ColaRec**  <br> [双流架构图] <br> *加入约束机制确保生成的GID有效,基于GID的树结构约束，每个位置只能生成有效子树中的token。* |
...
| **Re-ranking / Scoring** | **scoring** |  **ContRec** <br> <br> *根据向量相似度+语义匹配排出顺序，取top-k*  |
